...
describe 'Calculating sum of transaction amounts' do
  ...
  let(:transaction1) do
    subject.create_transaction(name: 'T1', account_id: account.id, amount: 100.0, currency: :usd, day_of_month: 1)
  end
  let(:transaction2) do
    subject.create_transaction(name: 'T2', account_id: account.id, amount: 150.0, currency: :usd, day_of_month: 2)
  end
